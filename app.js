...
var server = http.createServer(function (request, response){
    ...
    else if(request.url === '/dojo/ninja'){
    ...
    else if(request.url === '/img/img1.png'){
        // notice we won't include the utf8 encoding
        fs.readFile('./img/img1.png', function(errors, contents){
            response.writeHead(200, {'Content-type': 'image/jpg'});
            response.write(contents);
            response.end();
        })
    }
    else if(request.url === '/img/img2.png'){
        // notice we won't include the utf8 encoding
        fs.readFile('./img/img2.png', function(errors, contents){
            response.writeHead(200, {'Content-type': 'image/jpg'});
            response.write(contents);
            response.end();
        })
